...
Методы и сеттеры:  
`set contentElement(value: HTMLElement)` - задает содержимое модального окна.  
...
`set price(value: number | null)` - устанавливает цену товара: если передано число - отображается в формате "N синапсов", а если null — отображается "Бесценно".  
`set id(value: string)` - сохраняет идентификатор товара в data-id контейнера карточки.  
`get id(): string` - возвращает идентификатор товара из data-id.  
...
Сеттеры:  
`set product(value: IProduct)` - заполняет карточки данными товара (айди, заголовком и ценой).  
...
`categoryMap: Record<string, string>` - словарь соответствия названия категории CSS-модификатору.  

Сеттеры:  
`set categoryName(value: string)` - устанавливает текст категории и добавляет соответсвующий CSS-модификатор.  
...
`category: HTMLElement` - элемент отображения категории.  
`inBasket: boolean` - флаг, указывающий, находится ли товар в корзине.  
`categoryMap: Record<string, string>` - словарь соответствия названия категории CSS-модификатору.  

Методы и сеттеры:  
`set basketState(value: boolean)` - устанавливает состояние товара (в корзине или нет) и обновляет текст кнопки.  
`toggleBasket()` - переключает состояние кнопки в зависимости от наличия товара в корзине.  
`updateButton()` - обновляет внешний вид и состояние кнопки, если товар недоступен для покупки.  
`set categoryName(value: string)` - устанавливает текст категории и добавляет соответсвующий CSS-модификатор.  
...
Методы и сеттеры:  
`showErrors(errors: Record<string, string | undefined>)` - отображает ошибки в errorsElement.  
`abstract validate(): Record<string, string | undefined>` - возвращает объект ошибок для полей формы.  
`updateValidity()` - показывает ошибки.  
`onSubmit()` - вызывает событие с именем формы.  
`set valid(value: boolean)` - устанавливает доступность кнопки отправки.  
...
Методы и сеттеры:  
`setPayment(method: string)` - обновляет внешний вид кнопок оплаты.  
`validate(): Record<string, string | undefined>` - проверяет заполнение полей формы, возвращает объект с ошибками только для полей данного класса.  
`onSubmit()` - вызывает событие с именем формы.  
